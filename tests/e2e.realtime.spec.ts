...
test.describe('Real‑time verification suite', () => {
  ...
  test('Room creation and payload verification', async ({ page }) => {
    ...
    await page.click('[data-testid="create-room-button"]', { force: true });
  ...
  test('Join request flow', async ({ page, browser }) => {
    ...
    // User A creates room
    await pageA.click('[data-testid="create-room-button"]', { force: true });
    ...
    await pageB.click('[data-testid="create-room-button"]', { force: true });
    await pageB.fill('[data-testid="join-room-input"]', 'join-test-room');
    await pageB.click('[data-testid="join-request-button"]', { force: true });

    // User A receives request and accepts
    await pageA.waitForSelector('[data-testid="join-request-notification"]');
    await pageA.click('[data-testid="accept-join-button"]', { force: true });
    ...
    for (let i = 0; i < deviceDescriptors.length; i++) {
      for (let j = i + 1; j < deviceDescriptors.length; j++) {
        const ctxA = await browser.newContext({
          ...deviceDescriptors[i].options,
          permissions: ['microphone', 'camera']
        });
        const ctxB = await browser.newContext({
          ...deviceDescriptors[j].options,
          permissions: ['microphone', 'camera']
        });
        ...
        // B requests join
        await pageB.click('[data-testid="create-room-button"]', { force: true });
        await pageB.fill('[data-testid="join-room-input"]', `webrtc-${deviceDescriptors[i].name}-${deviceDescriptors[j].name}`);
        await pageB.click('[data-testid="join-request-button"]', { force: true });
        await pageA.waitForSelector('[data-testid="join-request-notification"]');
        await pageA.click('[data-testid="accept-join-button"]', { force: true });
        ...
        results.push({ pair: `${deviceDescriptors[i].name}-${deviceDescriptors[j].name}`, iceStates, hasRelay });
        await pageA.screenshot({ path: `artifacts/screenshots/webrtc_${deviceDescriptors[i].name}_${deviceDescriptors[j].name}.png` });
        await ctxA.close();
        await ctxB.close();
      }
    }
